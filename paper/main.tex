...
\begin{abstract}
Collaborative multimedia copy review often depends on evidence held by different platforms, rights holders, archives, and regional monitoring clients. Centralizing raw video, audio, embeddings, labels, or infringement records can expose copyrighted content, private moderation decisions, business-sensitive evidence, and user metadata. This paper presents \ours, a raw-media non-export fixed-evidence calibration and audit layer that sits above existing visual-copy, audio-fingerprint, descriptor, or retrieval pipelines. The method does not introduce a new visual localizer, retrieval model, audio fingerprint, or ownership-adjudication mechanism; instead, it asks whether compact query-reference evidence can be calibrated, personalized, protected, and audited across non-IID clients without exporting raw media or raw embeddings. The default head is a standardized logistic calibrator over precomputed score terms, reliability-weighted score terms, and cross-modal products; client/category fields are excluded by default. Training is evaluated with centralized, local, FedAvg, FedProx, SecureAgg simulation, a quantized transport proxy, real 2048-bit Paillier compact-update summation, personalization, and FedOpt-style baselines. On a checksum-verified, asset-disjoint VCSL public-label tier, protected federated calibration matches the corresponding plain compact-head baseline within the reported precision; separate descriptor and audio tiers are retained only as exploratory archived context. Additional audits report PR-AUC, ROC-AUC, FPR at 95\% recall, Brier score, calibration error, review workload, query-level ranking, poisoning stress, signed-receipt verification, and cryptographic overhead. High-recall visual false-positive rates remain poor at realistic prevalence. The evidence therefore supports human-review triage and signed audit logging, not automated enforcement, formal privacy, synchronized audiovisual learning, or superiority over end-to-end copy-localization systems.
\end{abstract}
