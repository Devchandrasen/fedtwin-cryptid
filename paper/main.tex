...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.76\textwidth]{figures/Figure1_FedTwin_CryptID_Framework_tmm.png}
...
\midrule
Synthetic conflict stress & Generated pair features & A+V & 20 & 15,000 & 5,000 & Controlled pipeline, FL, privacy, and ledger validation; not natural synchronized A/V evidence. \\
...
VCSL ISC visual & Released ISC frame descriptors & V & 11 & 6,000 & 3,000 & Visual-descriptor tier using variable-length 256-D frame descriptors. \\
FMA audio 20 s & FMA-small-compatible MP3 and official metadata & A & 8 & 12,000 & 6,000 & Audio-only transformed-copy and same-genre hard-negative tier. \\
FMA audio 5 s & FMA-small-compatible short clips & A & 8 & 12,000 & 6,000 & Short-clip stress tier for partial audio evidence. \\
\bottomrule
\end{tabularx}
\end{table*}

Table~\ref{tab:benchmark-tiers} summarizes the evaluation. The synthetic tier is a system-validation stress and is not presented as the main scientific evidence. In every reported real-data tier, complete positive connected components are assigned to frozen train/test partitions, negatives are sampled only within a partition, duplicate directional pairs are rejected, and zero asset-identity overlap is verified. VCSL ISC descriptors and FMA audio provide separate visual and audio evidence; they do not establish synchronized audiovisual performance. The FMA runs use 1,200 deterministically selected, successfully decoded tracks from 1,584 tracks extracted from the first three pinned mirror shards and joined to official FMA metadata. The mirror maintainer reports omitting six unreadable and 78 license-unclear files from the full redistribution pack. Runs use seeds 31, 37, and 41. The comparison includes random scoring, single-modality similarity, centralized heads, local-only training, FedAvg, FedProx, SecureAgg-simulation FedAvg, quantized-proxy FedAvg, Paillier update summation, FedAdam/FedYogi, and personalized FedAvg. We report PR-AUC, ROC-AUC, FPR at 95\% recall, Brier score, 15-bin expected calibration error, Recall@$K$, mAP, recall at fixed review budgets, and expected human-review workload. Confidence intervals cluster resamples by query; paired permutation and bootstrap comparisons use the same held-out examples and Holm correction across method comparisons. Exact client, pair, positive, negative, asset, query, and decode counts are released with each result row.
...
\begin{figure*}[!t]
  ...
  \caption{Three-seed PR-AUC (mean and sample standard deviation) for four confirmatory fixed-evidence tiers. VCSL and FMA are separate visual and audio evaluations and are not directly rank-comparable. SecureAgg is a protocol simulation and the quantized mode is a transport proxy.}
  \label{fig:multitier-pr-auc}
\end{figure*}

\begin{table*}[!t]
\centering
\caption{Confirmatory fixed-evidence calibration results. Values are mean PR-AUC over seeds 31, 37, and 41; the final column reports the best overall mean and sample standard deviation.}
\label{tab:main-results}
\scriptsize
\begin{tabularx}{\textwidth}{p{0.15\textwidth} p{0.17\textwidth} r r r r r r}
\toprule
Tier & Best single branch & Centralized & FedAvg & SecureAgg sim. & Quant. proxy & Best local/adapted & Best overall \\
\midrule
VCSL public labels & 0.8974 & 0.9183 & 0.9186 & 0.9186 & 0.9186 & 0.9175 & \textbf{0.9186}$\pm$0.0254 \\
VCSL ISC visual & 0.9115 & 0.9095 & 0.9093 & 0.9093 & 0.9093 & \textbf{0.9149} & \textbf{0.9149}$\pm$0.0619 \\
FMA audio 20 s & 0.9768 & 0.9943 & 0.9941 & 0.9941 & 0.9941 & \textbf{0.9945} & \textbf{0.9945}$\pm$0.0008 \\
FMA audio 5 s & 0.9807 & 0.9954 & 0.9954 & 0.9954 & 0.9954 & \textbf{0.9956} & \textbf{0.9956}$\pm$0.0007 \\
\bottomrule
\end{tabularx}
\end{table*}

All four rows in Fig.~\ref{fig:multitier-pr-auc} and Table~\ref{tab:main-results} are confirmatory, input-recorded, asset-disjoint reruns. On VCSL public labels, centralized calibration obtains 0.9183 PR-AUC; plain FedAvg, SecureAgg simulation, and the quantized transport proxy each obtain 0.9186 after rounding, and FedProx is also 0.9186. On released VCSL ISC descriptors, the local head is strongest at 0.9149, but the three-seed variability is large ($0.0619$ sample standard deviation). On 20-s FMA audio, the local audio-evidence head reaches 0.9945; on 5-s audio, personalized FedAvg reaches 0.9956. Protected modes track plain FedAvg to the displayed precision in every tier, preserving compact-head utility without establishing an accuracy gain or cross-tier superiority.

\begin{figure*}[!t]
  \centering
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/personalization_vs_local.pdf}
  \end{minipage}\hfill
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/vcsl_isc_client_variability.pdf}
  \end{minipage}\hfill
  \begin{minipage}{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/threshold_stability_by_client.pdf}
  \end{minipage}
  \caption{Confirmatory supporting views. Left: personalization relative to local/global references. Middle: VCSL ISC client variability. Right: client threshold stability. Wide intervals and high-recall false-positive rates bound deployment claims.}
...
\begin{table*}[!t]
\centering
\caption{Confirmatory VCSL public-label calibration and feature-policy ablation over seeds 31, 37, and 41. The invariant policy excludes direct client/category context fields.}
...
\midrule
Logistic poly2 invariant & \textbf{0.9116}$\pm$0.0226 & 0.3719 & Default invariant & \textbf{0.9116}$\pm$0.0226 \\
Logistic score-only & 0.9116$\pm$0.0221 & 0.3735 & Score only & 0.9116$\pm$0.0221 \\
FedYogi & 0.9077$\pm$0.0236 & 0.3816 & All fields minus context & 0.8997$\pm$0.0278 \\
Hist. gradient boosting & 0.9051$\pm$0.0252 & 0.4002 & Visual+temporal & 0.8938$\pm$0.0299 \\
FedAdam & 0.9025$\pm$0.0281 & 0.3900 & Remove reliability & 0.8911$\pm$0.0347 \\
Mean-score fusion & 0.8967$\pm$0.0115 & 0.4167 & All fields & 0.8886$\pm$0.0359 \\
Video similarity & 0.8912$\pm$0.0298 & 0.4328 & Audio only & 0.8713$\pm$0.0335 \\
\bottomrule
\end{tabularx}
\end{table*}

Table~\ref{tab:audit-ablation} isolates the calibration layer under fixed pair evidence using 30,000 training and 15,000 test pairs per seed. The invariant head exceeds score-only calibration by only 0.0008 pooled PR-AUC (paired-bootstrap 95\% CI $[0.0001,0.0015]$, Holm-adjusted $p=0.0208$), a statistically detectable but practically small difference. The score-only 20-D map uses video, audio, temporal, maximum, and mean scores plus squares and pairwise products. The default invariant map replaces direct context with reliability-weighted and modality-product terms. All-fields calibration is weaker, supporting exclusion of direct client/category fields from the default detector. The empirical claim remains modest: \ours{} provides leakage-aware federated calibration and signed audit logging over fixed evidence, not a new descriptor, retrieval method, or universally superior evidence fusion rule.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/fig10_feature_ablation.pdf}
  \caption{Confirmatory feature-policy ablation. Error bars are the sample standard deviation over seeds 31, 37, and 41; all plotted values are finite and regenerate from the released table.}
...
\begin{figure*}[!t]
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/review_workload_vs_prevalence.pdf}
  \end{minipage}
...
VCSL ISC visual & Video similarity & 0.9816 & 0.7460 & 0.6007 \\
VCSL ISC visual & Personalized FedAvg & 0.9787 & 0.7469 & 0.5971 \\
VCSL ISC visual & Local multimodal & 0.9854 & 0.7264 & 0.5756 \\
FMA audio 5 s & Audio similarity & 0.5168 & 1.0000 & 0.1520 \\
FMA audio 5 s & SecureAgg FedAvg & 0.9268 & 0.9874 & 0.0188 \\
FMA audio 5 s & Personalized FedAvg & 0.9311 & 0.9870 & 0.0160 \\
\bottomrule
\end{tabularx}
\end{table*}
...
Paillier compact sum & 8 clients, 24-D updates, 2048-bit key & $64\times$ bytes, $3.14\times10^{-7}$ max. error & Three-seed sums: enc. $14.70\pm0.38$ s; agg./dec. $1.455\pm0.014$ s (mean $\pm$ sample s.d.). \\
...
OpenAI Codex (GPT-5) assisted with language revision throughout the manuscript, code and test development, and table/figure preparation. The research framing, experimental design, interpretation, and final responsibility remain with the author; AI-assisted material was checked against cited sources, released evidence, and executable tests.
